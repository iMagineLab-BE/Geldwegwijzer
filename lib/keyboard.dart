...
class NumericKeyboard extends StatelessWidget {
  ...
  void _addChar(Object value) {
    _editorController.text = _validateChar(
        _editorController.text, _editorController.text + value.toString());
  }

  void _removeChar() {
    if (_editorController.text.length > 1) {
      _editorController.text = _editorController.text
          .substring(0, _editorController.text.length - 1);
    } else {
          ...
          constraints: BoxConstraints.tightForFinite(
              width: _isHorizontal()
                  ...
                  : (SizeConfig.screenWidth <= 800
                      ? double.maxFinite
                      : SizeConfig.blockSizeHorizontal * 80.0)),
                ...
                child: FittedBox(
                    child: Padding(
                      child: Icon(Icons.backspace,
                          size: SizeConfig.blockSizeHorizontal * 100),
                      padding: EdgeInsets.all(isHorizontal()
                          ? SizeConfig.blockSizeVertical * 15.0
                          : SizeConfig.blockSizeHorizontal * 10.0),
                    ),
                    fit: BoxFit.scaleDown)),
            elevation: 3,
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(isHorizontal()
        ...
        child: RaisedButton(
            ...
            child: FittedBox(
              child: Text(
                value.toString(),
                style: TextStyle(
                    fontSize: SizeConfig.blockSizeHorizontal * 100,
                    color: Colors.white),
              ),
              fit: BoxFit.scaleDown,
            ),
